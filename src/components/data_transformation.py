...
class DataTransformation:
    ...
    def initiate_data_transformation(self,X_train_path,X_test_path,y_train_path,y_test_path):
        try:
            ...
            TOP_FLIGHT_CODE=X_train['flight_code'].value_counts().iloc[:400].index.tolist()
            X_train['flight_code']=np.where(X_train['flight_code'].isin(TOP_FLIGHT_CODE), X_train['flight_code'], 'Other')
            X_test['flight_code']=np.where(X_test['flight_code'].isin(TOP_FLIGHT_CODE), X_test['flight_code'], 'Other')
